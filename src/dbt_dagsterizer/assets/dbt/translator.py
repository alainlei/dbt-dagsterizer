...
class LubanDagsterDbtTranslator(DagsterDbtTranslator):
    ...
    def get_automation_condition(self, dbt_resource_props):
        ...
        is_hourly = bool(name) and self.partitions_by_model.get(
            str(name)) == "hourly"
        is_monthly = bool(name) and self.partitions_by_model.get(
            str(name)) == "monthly"
    ...
    def get_partitions_def(self, dbt_resource_props: Mapping[str, Any]) -> Optional[dg.PartitionsDefinition]:
        ...
        partition type (e.g., all daily, all hourly, all monthly, or all unpartitioned). If you have mixed partition
        ...
        if partition_type == "monthly":
            return self.monthly_partitions_def
