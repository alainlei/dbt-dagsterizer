...
- Automation conditions: See [Auto-Materialization Tags](#auto-materialization-tags) below
- Grouping and filtering in Dagster UI

### Auto-Materialization Tags

Models tagged with `dim` or `automation_table` are automatically materialized when observable sources detect new data in the database:

```sql
{{
  config(
    tags=["dim"]  -- or tags=["automation_table"]
  )
}}
```

**Behavior**:
- When an observable source detects new data (via watermark changes), Dagster automatically triggers materialization of models with these tags
...
**Example** (Dimension table with auto-materialization):
