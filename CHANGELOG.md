...
- **Cron-based schedules with partition control.** New `type: cron` schedule entries in `dagsterization.yml` (created with `meta schedule --schedule-type cron`) cover cadences the `daily_at`/`hourly_at`/`monthly_at` presets cannot express — every 15 minutes, weekday mornings only, the last day of the month, etc.
  - New `cron()` preset in `schedules/dbt/presets.py` alongside the three existing presets, and a new top-level `cron.py` module with `validate_cron_expression()` / `cron_expression_warnings()` shared by the CLI, `orchestration_config.set_schedule()`, and `meta validate`, so an invalid expression fails fast with an error naming the offending field instead of surfacing at Dagster definition-build time.
  - The validator accepts the five-field crontab syntax Dagster accepts: `*`, lists (`1,15`), ranges (`9-17`), steps (`*/15`, `5/10`), month/day names (`JAN-DEC`, `SUN-SAT`), `L` (last day of the month), `weekday#nth` (e.g. `5#2` = second Friday), `?` in the day fields, and `@daily`-style macros. Out-of-range values, wrong field counts, inverted ranges, and never-firing expressions such as February 30th are rejected. A step larger than its field range (e.g. `*/90 * * * *`) is accepted with a warning — Dagster normalizes it and the schedule fires more often than the step suggests.
  - Cron schedules declare the partition window they target via `partition_type: daily|hourly|monthly|unpartitioned` (default `daily`). The window is derived from the tick time exactly like the presets, so `*/15 * * * *` with `partition_type: daily` and `offset_days: 1` runs the previous day's partition on every tick. Offset/lookback fields belonging to another granularity are rejected at validation, preset, and factory level (e.g. `offset_hours` on a `partition_type: daily` cron schedule is an error).
  - `unpartitioned` cron schedules are the first unpartitioned schedule type: each tick launches one run via a plain `RunRequest` with no `run_key` (a constant `run_key` would dedupe all later ticks away), and they must not set any offset/lookback fields.
  - Cron schedules execute in the global `timezone` from `dagsterization.yml` (default `UTC`), like the other schedule types.
  - CLI: `meta schedule --schedule-type cron --cron-expression "*/15 * * * *" --partition-type daily --offset-days 1`; `--cron-expression` and `--partition-type` are rejected when the schedule type is not `cron`.
  - Test coverage: new `tests/test_cron_expression.py` (222 lines) and `tests/test_cron_schedules.py` (783 lines) modules plus cron round-trips in `tests/test_cli_meta_generation.py`, covering expression validation, cross-granularity rejection, factory partition keys for all four partition types, and CLI flag guards.
  - Docs: new Cron Schedule section in `docs/concepts/dagsterization-yml.md` with YAML and CLI equivalents; `docs/concepts/cli.md` documents the new flags and a cron example.
...
- CLI `meta schedule --minute` is now required only for the preset schedule types (`daily_at`, `hourly_at`, `monthly_at`) and unused for `cron` (previously it was unconditionally required). `set_schedule()` in `orchestration_config.py` correspondingly accepts optional `hour`/`minute` — required for preset types — plus new `cron_expression` and `partition_type` parameters, and cron entries store `cron_expression`/`partition_type` instead of `hour`/`minute`. Validation error messages for unknown schedule types now name `cron`.
